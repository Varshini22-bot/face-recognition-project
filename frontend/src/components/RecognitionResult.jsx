...
export default function RecognitionResult({ file, previewUrl, result, loading, error, onRecognize, onUpload }) {
  ...
  return (
    <section className="result-panel panel-surface" aria-labelledby="result-title">
      ...
      {result && <><div className="result-face-list">{result.faces.map((face, index) => <div className="result-face-row" key={`${face.name || 'unknown'}-${index}`}><strong>{face.name || 'Unknown person'}</strong><span className={face.recognized ? 'known-text' : 'unknown-text'}>{face.status} · {face.similarity == null ? '—' : `${(face.similarity * 100).toFixed(1)}%`}</span></div>)}</div><div className="result-facts"><div><span>Faces</span><strong>{result.face_count}</strong></div><div><span>Time</span><strong>{result.processing_time_ms} ms</strong></div><div><span>Threshold</span><strong>{firstFace?.threshold?.toFixed(2) || '0.50'}</strong></div><div><span>Decision</span><strong>{firstFace?.status === 'known' ? 'Accepted' : 'Rejected'}</strong></div></div></>}
